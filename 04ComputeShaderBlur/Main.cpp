...
//——————————————————————————————————————————————
	...
	try
	{
		...
		//14、创建默认堆资源
		{
			...
			//———————————————————————————————————————————————————————————
			//创建默认堆及其资源，采用Committed创建方式，压根不需要上传堆，因为这俩资源都是来自后台缓冲区或者compute shader的
		...
		for (int i = 0; i < 3; i++) {
			...
			//之前的End我们可以删除了，因为唯一使用的地方被注释掉了，具体看下面循环
		...
		//————————————————————————————————————————————————————
		//修正上一节的bug，如果不写这两句，在第一轮循环中添加Drawcall命令的是命令列表[1]，他并没有绑定PSO，
		//所以会使用默认的，而我们写了下面的一句，就会使用我们指定的PSO。
		...
		while (msg.message != WM_QUIT)
		{
			...
			else
			{
				...
				//——————————————————————————————————————————————————————
				//此处代码需要注释掉，因为我们Drawcall之后还没有实际完成绘制，还不能直接把渲染状态修改为Present
...
//————————————————————————————————————————————————————————————————————————————				
//主循环内的代码修改都在这里，因为我们的模糊操作只是在原来的录制命令基础上增加一系列命令而已
				...
				//          把后台缓冲区复制到pIBlurMap0
				//他需要先把后台缓冲区和pIBlurMap0的状态确定一下，以防冲突，也就是上边两行
				CD3DX12_RESOURCE_BARRIER stBackCopyToTextureBarrier = CD3DX12_RESOURCE_BARRIER::Transition(pIARenderTargets[nFrameIndex].Get(),
					D3D12_RESOURCE_STATE_RENDER_TARGET, D3D12_RESOURCE_STATE_COPY_SOURCE);
				pICMDListArr[nextFrameResource]->ResourceBarrier(1, &stBackCopyToTextureBarrier);
				CD3DX12_RESOURCE_BARRIER stAcceptBackBufferBarrier = CD3DX12_RESOURCE_BARRIER::Transition(pIBlurMap0.Get(),
					D3D12_RESOURCE_STATE_COMMON, D3D12_RESOURCE_STATE_COPY_DEST);
				pICMDListArr[nextFrameResource]->ResourceBarrier(1, &stAcceptBackBufferBarrier);

				//执行复制动作
				pICMDListArr[nextFrameResource]->CopyResource(pIBlurMap0.Get(), pIARenderTargets[nFrameIndex].Get());

				//为接下来的计算着色器的调用准备，把资源的状态进行调整。
				...
				for (int i = 0; i < blurTimes; i++) {
					...
					//————————————————————————————————————
					//水平模糊部分，先确定出四个描述符的位置，接下来垂直和水平时都要使用。
					...
					CD3DX12_RESOURCE_BARRIER D = CD3DX12_RESOURCE_BARRIER::Transition(pIARenderTargets[nFrameIndex].Get(),
						D3D12_RESOURCE_STATE_COPY_SOURCE, D3D12_RESOURCE_STATE_COPY_DEST);
					pICMDListArr[nextFrameResource]->ResourceBarrier(1, &D);
					//————————————————————————————————————
					...
					//下面是为了循环做准备的，因为下一轮for循环，需要pIBlurMap1的初始状态为D3D12_RESOURCE_STATE_UNORDERED_ACCESS
					//同时后台缓冲区也是同样的，下一轮for循环，他最开始应该是COPY_SOURCE状态
					CD3DX12_RESOURCE_BARRIER E = CD3DX12_RESOURCE_BARRIER::Transition(pIARenderTargets[nFrameIndex].Get(),
						D3D12_RESOURCE_STATE_COPY_DEST, D3D12_RESOURCE_STATE_COPY_SOURCE);
					pICMDListArr[nextFrameResource]->ResourceBarrier(1, &E);
				...
				//我们的后台缓冲区下一轮循环会被Present，也是为了衔接需要转换成PRESENT。
