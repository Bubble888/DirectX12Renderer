...
int APIENTRY _tWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPWSTR    lpCmdLine, int nCmdShow)
{
	...
	try
	{
		...
		//10、创建SRV和CBV堆
		{
			...
			stSRVHeapDesc.NumDescriptors = 4;
...

		{
			...
			GRS_THROW_IF_FAILED(D3DCompileFromFile(pszShaderFileName, nullptr, nullptr
				, "VSMain", "vs_5_0", compileFlags, 0, &pIBlobVertexShaderDeferred, nullptr));
			GRS_THROW_IF_FAILED(D3DCompileFromFile(pszShaderFileName, nullptr, nullptr
		...
		//11、创建根签名
		{
			...
			D3DReflect(pIBlobPixelShaderDeferred->GetBufferPointer(), pIBlobPixelShaderDeferred->GetBufferSize(), IID_ID3D12ShaderReflection, (void**)&Reflection);
			...
			for (UINT i = 0; i < ShaderDesc.BoundResources; i++) {
				...
				if (ResourceType == D3D_SHADER_INPUT_TYPE::D3D_SIT_TEXTURE) {
						stDSPRanges1[0].NumDescriptors = BindCount;
						stDSPRanges1[0].BaseShaderRegister = BindPoint;
						stDSPRanges1[0].RegisterSpace = RegisterSpace;
				}
			}

			D3DReflect(pIBlobVertexShaderDeferred->GetBufferPointer(), pIBlobVertexShaderDeferred->GetBufferSize(), IID_ID3D12ShaderReflection, (void**)&Reflection);
			...
			for (UINT i = 0; i < ShaderDesc.BoundResources; i++) {
				...
				if (ResourceType == D3D_SHADER_INPUT_TYPE::D3D_SIT_CBUFFER) {
					stDSPRanges1[1].NumDescriptors = BindCount;
					stDSPRanges1[1].BaseShaderRegister = BindPoint;
					stDSPRanges1[1].RegisterSpace = RegisterSpace;
				}
			}
		...
		//12、编译Shader创建渲染管线状态对象
		{
			...
			stPSODesc.BlendState.RenderTarget[1].RenderTargetWriteMask = D3D12_COLOR_WRITE_ENABLE_ALL;
...
// Enable better shader debugging with the graphics debugging tools.
		...
		//20、创建顶点缓冲
		{
			...
			stVertexBufferViewDeferred.StrideInBytes = sizeof(ST_GRS_VERTEX);
			stVertexBufferViewDeferred.SizeInBytes = nVertexBufferSize;
		...
		//21、创建索引缓冲区		
		{
			...
			stIndexBufferViewDeferred.SizeInBytes = nszIndexBuffer;
		...
		while (msg.message != WM_QUIT)
		{
			...
			else
			{
				...
				ImGui::ColorEdit3("Clear Color", f4RTTexClearColor);
				ImGui::End();
				ImGui::Render();
				...
				//---------------------------------------------------------------------------------------------
				{
					...
					XMMATRIX xmMVP = XMMatrixMultiply(xmRot, view);
				...
				pICMDList->OMSetRenderTargets(2, &stRTVHandle, TRUE, nullptr);//改为TURE
