...
//Texture2D metallic : register(t0);
...
{
    PSOutput o;
    o.halfcolor0 = baseColor.Sample(g_sampler, input.uv) / 100.0f;
...
//    float3 Cdlin = mon2lin(baseColor.Sample(g_sampler, uv).xyz);
//    float Cdlum = .3 * Cdlin[0] + .6 * Cdlin[1] + .1 * Cdlin[2]; // luminance approx.
 
//    float3 Ctint = Cdlum > 0 ? Cdlin / Cdlum : float3(1, 1, 1); // normalize lum. to isolate hue+sat
//    float3 Cspec0 = lerp(_specular * .08 * lerp(float3(1, 1, 1), Ctint, specularTint), Cdlin, metallic.Sample(g_sampler, uv).x);
//    float3 Csheen = lerp(float3(1, 1, 1), Ctint, sheenTint);
 
//                // Diffuse fresnel - go from 1 at normal incidence to .5 at grazing
//                // and lerp in diffuse retro-reflection based on roughness
//    float FL = SchlickFresnel(NdotL), FV = SchlickFresnel(NdotV);
//    float Fd90 = 0.5 + 2 * LdotH * LdotH * roughness.Sample(g_sampler, uv).x;
//    float Fd = lerp(1.0, Fd90, FL) * lerp(1.0, Fd90, FV);
 
//                // Based on Hanrahan-Krueger brdf approximation of isotropic bssrdf
//                // 1.25 scale is used to (roughly) preserve albedo
//                // Fss90 used to "flatten" retroreflection based on roughness
//    float Fss90 = LdotH * LdotH * roughness.Sample(g_sampler, uv).x;
...
//    float ax = max(.001, sqr(roughness.Sample(g_sampler, uv).r) / aspect);
//    float ay = max(.001, sqr(roughness.Sample(g_sampler, uv).r) * aspect);
...
//    return ((1 / PI) * lerp(Fd, ss, subsurface) * Cdlin + Fsheen) * (1 - metallic.Sample(g_sampler, uv).r) + Gs * Fs * Ds + .25 * clearcoat * Gr * Fr * Dr;
//}
           
//void VSMain(inout float3 vertex : POSITION, inout float3 normal : NORMAL, inout float2 uv : TEXCOORD0, inout float4 tangent : TANGENT, out float3 world : TEXCOORD1)
//{
//    world = mul(m_ObjectToWorld, vertex).xyz;
//    vertex = mul(float4(vertex,1.0f), m_MVP);
//}
