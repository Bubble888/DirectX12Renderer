...
int APIENTRY _tWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPWSTR    lpCmdLine, int nCmdShow)
{
	...
	ComPtr<ID3D12PipelineState> pIKawaseBlurPSO;
	...
	ComPtr<ID3D12Resource>				pIMipmap;
	...
	try
	{
		...
		//10、创建描述符堆
		{
			...
			//为了新增的mipmap图，这里也需要多加1个描述符。只需要UAV
			stSRVHeapDesc.NumDescriptors = 9;
		...
		//11、创建根签名
		{
			...
			CD3DX12_DESCRIPTOR_RANGE mipmapUavTable;
			mipmapUavTable.Init(D3D12_DESCRIPTOR_RANGE_TYPE_UAV, 1, 1);
			...
			slotRootParameter[4].InitAsDescriptorTable(1, &mipmapUavTable);

			//由于描述符表要求描述符是连续的，这里根本实现不了，因为mipmap的srv有时候和map1的srv一起绑定
			//有时候和map2的srv一起绑定，那么就不能同时做到连续，所以还是把mipmap的单独放置。

			//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!44
			//! 这里注意，上边本来是把新加的两个描述符也放在了space0里面，但是这会导致下面的序列化根签名失败，
			//! 所以我就瞎猫碰上死耗子瞎改了一下这个space，改成了1，没想到就跑通了。
			//! 可能是一个space里面他不让你放类型一样的描述符表。
		...
		//12、创建PSO
		{
			...
			StringCchPrintf(pszShaderFileName, MAX_PATH, _T("%sShader\\DualBlur.hlsl"), pszAppPath);
			ComPtr<ID3DBlob> pIBlobComputeShaderKawase;
			GRS_THROW_IF_FAILED(D3DCompileFromFile(pszShaderFileName, nullptr, nullptr
				, "DualBlurCS", "cs_5_0", compileFlags, 0, &pIBlobComputeShaderKawase, nullptr));
			...
			horzBlurPSO.CS =
			{
				pIBlobComputeShaderKawase->GetBufferPointer(),
				pIBlobComputeShaderKawase->GetBufferSize()
			};
			horzBlurPSO.Flags = D3D12_PIPELINE_STATE_FLAG_NONE;
			GRS_THROW_IF_FAILED(pID3D12Device4->CreateComputePipelineState(&horzBlurPSO, IID_PPV_ARGS(&pIKawaseBlurPSO)));
		...
		//23、创建SRV资源视图
		{
			...
			stCPUHandle.Offset(1, nSRVDescriptorSize);
			pID3D12Device4->CreateUnorderedAccessView(pIMipmap.Get(), nullptr, &uavDesc, stCPUHandle);
			//————————————————————————————————————————————————————————————
		...
		//用户指定上边的那个数，其实执行的模糊次数是上边的两倍，因为升降是成对的，所以用户指定模糊多少对就好了
		...
		while (msg.message != WM_QUIT)
		{
			...
			else
			{
				...
				//需要设置完根签名之后再设置这个描述符表，否则有错。
				stGPUSRVHandle.ptr += nSRVDescriptorSize;
				pICMDListArr[nextFrameResource]->SetComputeRootDescriptorTable(1, stGPUSRVHandle);
				...
				//总思路：拿第一次举例，map0把数据进行四合一写到mipmap上，然后同步线程，接着读取mipmap进行模糊写到map1上，map1和map0交换角色

				// i为偶数的时候是pIBlurMap0作为CS输入，i为奇数的时候是pIBlurMap1作为CS输入
				// 根据正负来确定升还是降采样
				// 这里我们并不是实际把图片尺寸缩小，而是选择使用mipmap，如果选择把图缩小还得注意最小是1个像素。再除以2就成0了。但这里不用。
				// 
				// 负数是降采样，而0和正数是升采样。
				// 
				// 这样会有一个问题，就是我们刚开始说的奇偶就变了。-static_cast<int>(blurTimes)如果是奇数那map0就奇数开始，那就map0作为输入
				// 如果他是偶数那就map0在偶数时作为CS输入。
				//注意下面改成了带有负数的，在赋值包括比较的时候都需要换成int类型，否则结果并不是理想的。
				for (int i = -static_cast<int>(blurTimes); i < static_cast<int>(blurTimes); i++) {
					...
					stCPUHandle.Offset(1, nSRVDescriptorSize);
					CD3DX12_GPU_DESCRIPTOR_HANDLE mMipMapUav = stCPUHandle;

					pICMDListArr[nextFrameResource]->SetComputeRoot32BitConstants(0, 1, &i, 0);

					pICMDListArr[nextFrameResource]->SetPipelineState(pIKawaseBlurPSO.Get());

					//-blurTimes % 2 代表刚开始第一次的奇偶，下面我们取blurTimes % 2也是一样的，然后abs(i)%2就代表当前次的奇偶
					//如果说当前的奇偶等于第一次的奇偶，那就说明这次是map0被读而map1被写。
					pICMDListArr[nextFrameResource]->SetComputeRootDescriptorTable(4, mMipMapUav);
					if (abs(i) % 2 == (blurTimes % 2)) {
						...
						//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
						//! 这块曾经写了一个bug，主要是SetComputeRootDescriptorTable的时候，把本该在根参数1中的描述符，
						//! 它对应的下标写成了2，同时2写成了1，两个颠倒了，这就导致它根本找不着你的描述符，这就会导致资源的引用不上
						//! 但是这个他根本没有任何的错误信息，这里最后就是0缓冲区始终没有被写入，也就是他的src始终没有被绑定上导致的。
					...
					//下面故意多除以一个2，让一些方块没有线程
					if (i < 0) {
						UINT numGroupsX = (UINT)ceilf(iWidth / 32.0f);
						UINT numGroupsY = (UINT)ceilf(iHeight / 32.0f);
						pICMDListArr[nextFrameResource]->Dispatch(numGroupsX, numGroupsY, 1);
					}
					else {
						// 升采样的时候是先升然后再模糊。也就是刚开始需要一个线程处理一个map位置，搞到mipmap上的4个位置，
						// 然后模糊并输出。
						UINT numGroupsX = (UINT)ceilf(iWidth / 16.0f);
						UINT numGroupsY = (UINT)ceilf(iHeight / 16.0f);
						pICMDListArr[nextFrameResource]->Dispatch(numGroupsX, numGroupsY, 1);
					}
					if (abs(i) % 2 == (blurTimes % 2)) {
						...
						CD3DX12_RESOURCE_BARRIER A = CD3DX12_RESOURCE_BARRIER::Transition(pIBlurMap1.Get(),
							D3D12_RESOURCE_STATE_GENERIC_READ, D3D12_RESOURCE_STATE_UNORDERED_ACCESS);
						pICMDListArr[nextFrameResource]->ResourceBarrier(1, &A);

						CD3DX12_RESOURCE_BARRIER B = CD3DX12_RESOURCE_BARRIER::Transition(pIBlurMap0.Get(),
							D3D12_RESOURCE_STATE_UNORDERED_ACCESS, D3D12_RESOURCE_STATE_GENERIC_READ);
				...
				//实际循环一定进行了偶数次，所以最终的结果一定是存储在map0上的，所以之前这里的判断就不需要了
