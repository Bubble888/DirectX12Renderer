...
struct ST_GRS_FRAME_MVP_BUFFER
{
	...
	XMFLOAT4X4 m_MVPSkyBox;
	XMFLOAT4X4 m_MVPFloor;
	XMFLOAT4X4 m_ObjectToWorld;
	XMFLOAT4X4 m_ObjectToWorldCubemap;
	XMVECTOR m_ViewPos;
	XMFLOAT3 lightDir;
...
int APIENTRY _tWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPWSTR    lpCmdLine, int nCmdShow)
{
	...
	D3D12_VERTEX_BUFFER_VIEW			stVertexBufferViewCubemap = {};
	D3D12_INDEX_BUFFER_VIEW				stIndexBufferViewCubemap = {};
	D3D12_VERTEX_BUFFER_VIEW			stVertexBufferView = {};
	D3D12_INDEX_BUFFER_VIEW				stIndexBufferView = {};
	D3D12_VERTEX_BUFFER_VIEW			stVertexBufferViewFloor = {};
	D3D12_INDEX_BUFFER_VIEW				stIndexBufferViewFloor = {};
	...
	ComPtr<ID3DBlob> pIBlobVertexShaderCubemap;
	ComPtr<ID3DBlob> pIBlobPixelShaderCubemap;
	ComPtr<ID3DBlob> pIBlobVertexShaderFloor;
	ComPtr<ID3DBlob> pIBlobPixelShaderFloor;
	...
	ComPtr<ID3D12Resource>				pICubemap;
	ComPtr<ID3D12Resource>				pICubemapUpload;
	...
	ComPtr<ID3D12RootSignature>			pIRootSignatureCubemap;
	ComPtr<ID3D12RootSignature>			pIRootSignatureFloor;


	ComPtr<ID3D12PipelineState>			pIPipelineState;
	ComPtr<ID3D12PipelineState>			pIPipelineStateDeferred;
	ComPtr<ID3D12PipelineState>			pIPipelineStateCubemap;
	ComPtr<ID3D12PipelineState>			pIPipelineStateFloor;
	...
	ComPtr<ID3D12Resource>				pIVertexBufferCubemap;
	ComPtr<ID3D12Resource>				pIIndexBufferCubemap;
	ComPtr<ID3D12Resource>				pIVertexBuffer;
	ComPtr<ID3D12Resource>				pIIndexBuffer;
	ComPtr<ID3D12Resource>				pIVertexBufferFloor;
	ComPtr<ID3D12Resource>				pIIndexBufferFloor;
	...
	try
	{
		...
		//加载模型
		{
			...
			LoadModels("Resources/models/sphere.fbx");
		}
		...
		//6、枚举适配器，并选择合适的适配器来创建3D设备对象
		{
			...
			for (UINT nAdapterIndex = 0; DXGI_ERROR_NOT_FOUND != pIDXGIFactory5->EnumAdapters1(nAdapterIndex, &pIAdapter1); ++nAdapterIndex)
			{
				...
				if (stAdapterDesc.Flags & DXGI_ADAPTER_FLAG_SOFTWARE || nAdapterIndex == 0)
				{//跳过软件虚拟适配器设备
					continue;
				}
...

		{
			...
			StringCchPrintf(pszTextureFileName, MAX_PATH, _T("%sTexture\\%s"), pszAppPath, _T("snowcube1024.dds"));

			GRS_THROW_IF_FAILED(CreateDDSTextureFromFile12(pID3D12Device4.Get(),
				pICMDList.Get(), pszTextureFileName,
				pICubemap, pICubemapUpload));
		}
		...
		//10、创建SRV和CBV堆
		{
			...
			stSRVHeapDesc.NumDescriptors = 8;//多一个第八个是用来Cubemap的。
		...
		//编译shader
		{
			...
			GRS_THROW_IF_FAILED(D3DCompileFromFile(pszShaderFileName, nullptr, nullptr
				, "VSMainCubemap", "vs_5_0", compileFlags, 0, &pIBlobVertexShaderCubemap, &error));
			//::OutputDebugStringA((char*)error->GetBufferPointer());
			GRS_THROW_IF_FAILED(D3DCompileFromFile(pszShaderFileName, nullptr, nullptr
				, "PSMainCubemap", "ps_5_0", compileFlags, 0, &pIBlobPixelShaderCubemap, nullptr));



			GRS_THROW_IF_FAILED(D3DCompileFromFile(pszShaderFileName, nullptr, nullptr
				, "VSMainFloor", "vs_5_0", compileFlags, 0, &pIBlobVertexShaderFloor, &error));
			//::OutputDebugStringA((char*)error->GetBufferPointer());
			GRS_THROW_IF_FAILED(D3DCompileFromFile(pszShaderFileName, nullptr, nullptr
				, "PSMainFloor", "ps_5_0", compileFlags, 0, &pIBlobPixelShaderFloor, nullptr));
		...
		//11、创建根签名
		{
			...
			//————————————————————————————————————————————————————
			//cubemap
			D3D12_ROOT_PARAMETER1 stRootParameters3[2] = {};
			//由于我们的这个shader使用了变换的常量缓冲区，这里虽只用了一个矩阵没有用其他变量，但是仍然需要在根签名中写上这个缓冲区。
			stDSPRanges1[0].RangeType = D3D12_DESCRIPTOR_RANGE_TYPE_SRV;
			stDSPRanges1[0].Flags = D3D12_DESCRIPTOR_RANGE_FLAG_DATA_STATIC_WHILE_SET_AT_EXECUTE;
			stDSPRanges1[0].OffsetInDescriptorsFromTableStart = 0;
			stDSPRanges1[0].NumDescriptors = 1;
			stDSPRanges1[0].BaseShaderRegister = 4;
			stDSPRanges1[0].RegisterSpace = 0;
			//CBV
			stDSPRanges1[1].NumDescriptors = 1;
			stDSPRanges1[1].BaseShaderRegister = 0;
			...
			//SRV
			stRootParameters3[0].ParameterType = D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE;
			stRootParameters3[0].ShaderVisibility = D3D12_SHADER_VISIBILITY_PIXEL;
			stRootParameters3[0].DescriptorTable.NumDescriptorRanges = 1;
			stRootParameters3[0].DescriptorTable.pDescriptorRanges = &stDSPRanges1[0];
			//CBV
			stRootParameters3[1].ParameterType = D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE;
			stRootParameters3[1].ShaderVisibility = D3D12_SHADER_VISIBILITY_ALL;		//CBV是所有Shader可见
			stRootParameters3[1].DescriptorTable.NumDescriptorRanges = 1;
			stRootParameters3[1].DescriptorTable.pDescriptorRanges = &stDSPRanges1[1];

			stRootSignatureDesc.Desc_1_1.NumStaticSamplers = _countof(stSamplerDesc);
			stRootSignatureDesc.Desc_1_1.pStaticSamplers = stSamplerDesc;
			stRootSignatureDesc.Desc_1_1.NumParameters = _countof(stRootParameters3);
			stRootSignatureDesc.Desc_1_1.pParameters = stRootParameters3;
			...
			GRS_THROW_IF_FAILED(pID3D12Device4->CreateRootSignature(0
				, pISignatureBlob->GetBufferPointer()
				, pISignatureBlob->GetBufferSize()
				, IID_PPV_ARGS(&pIRootSignatureCubemap)));

			//————————————————————————————————————————————————————
			//floor
			D3D12_ROOT_PARAMETER1 stRootParameters4[1] = {};
			//CBV
			stDSPRanges1[0].NumDescriptors = 1;
			stDSPRanges1[0].BaseShaderRegister = 0;
			stDSPRanges1[0].RegisterSpace = 0;
			stDSPRanges1[0].RangeType = D3D12_DESCRIPTOR_RANGE_TYPE_CBV;
			stDSPRanges1[0].Flags = D3D12_DESCRIPTOR_RANGE_FLAG_NONE;
			stDSPRanges1[0].OffsetInDescriptorsFromTableStart = 0;

			//CBV
			stRootParameters4[0].ParameterType = D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE;
			stRootParameters4[0].ShaderVisibility = D3D12_SHADER_VISIBILITY_ALL;		
			stRootParameters4[0].DescriptorTable.NumDescriptorRanges = 1;
			stRootParameters4[0].DescriptorTable.pDescriptorRanges = &stDSPRanges1[0];

			stRootSignatureDesc.Desc_1_1.NumStaticSamplers = _countof(stSamplerDesc);
			stRootSignatureDesc.Desc_1_1.pStaticSamplers = stSamplerDesc;
			stRootSignatureDesc.Desc_1_1.NumParameters = _countof(stRootParameters4);
			stRootSignatureDesc.Desc_1_1.pParameters = stRootParameters4;
			...
			GRS_THROW_IF_FAILED(pID3D12Device4->CreateRootSignature(0
				, pISignatureBlob->GetBufferPointer()
				, pISignatureBlob->GetBufferSize()
				, IID_PPV_ARGS(&pIRootSignatureFloor)));
		...
		//12、编译Shader创建渲染管线状态对象
		{
			...
			stPSODesc.pRootSignature = pIRootSignatureFloor.Get();
			stPSODesc.VS.pShaderBytecode = pIBlobVertexShaderFloor->GetBufferPointer();
			stPSODesc.VS.BytecodeLength = pIBlobVertexShaderFloor->GetBufferSize();

			stPSODesc.PS.pShaderBytecode = pIBlobPixelShaderFloor->GetBufferPointer();
			stPSODesc.PS.BytecodeLength = pIBlobPixelShaderFloor->GetBufferSize();
			GRS_THROW_IF_FAILED(pID3D12Device4->CreateGraphicsPipelineState(&stPSODesc
				, IID_PPV_ARGS(&pIPipelineStateFloor)));


			//cubemap
			//使用第一个pass的输入布局，这样就不必重新写load模型的部分了。
			stPSODesc.InputLayout = { stInputElementDescsDeferred, _countof(stInputElementDescsDeferred) };
			stPSODesc.pRootSignature = pIRootSignatureCubemap.Get();
			stPSODesc.VS.pShaderBytecode = pIBlobVertexShaderCubemap->GetBufferPointer();
			stPSODesc.VS.BytecodeLength = pIBlobVertexShaderCubemap->GetBufferSize();

			stPSODesc.PS.pShaderBytecode = pIBlobPixelShaderCubemap->GetBufferPointer();
			stPSODesc.PS.BytecodeLength = pIBlobPixelShaderCubemap->GetBufferSize();
			stPSODesc.RasterizerState.CullMode = D3D12_CULL_MODE_NONE;
			GRS_THROW_IF_FAILED(pID3D12Device4->CreateGraphicsPipelineState(&stPSODesc
				, IID_PPV_ARGS(&pIPipelineStateCubemap)));
...
// Enable better shader debugging with the graphics debugging tools.
		...
		//20、创建顶点缓冲
		{
			...
			//——————————————————————————————————————————————————————————
			//floor
			const UINT floorVertexSize = sizeof(floorVertex);
			stResSesc.Width = floorVertexSize;
			...
			GRS_THROW_IF_FAILED(pID3D12Device4->CreateCommittedResource(
				...
				D3D12_RESOURCE_STATE_GENERIC_READ,
				nullptr,
				IID_PPV_ARGS(&pIVertexBufferFloor)));

			GRS_THROW_IF_FAILED(pIVertexBufferFloor->Map(0, &stReadRange, reinterpret_cast<void**>(&pVertexDataBegin)));
			memcpy(pVertexDataBegin, floorVertex, sizeof(floorVertex));
			pIVertexBufferFloor->Unmap(0, nullptr);

			stVertexBufferViewFloor.BufferLocation = pIVertexBufferFloor->GetGPUVirtualAddress();
			stVertexBufferViewFloor.StrideInBytes = sizeof(ST_GRS_VERTEX);
			stVertexBufferViewFloor.SizeInBytes = floorVertexSize;
			//——————————————————————————————————————————————————————————
		...
		//21、创建索引缓冲区		
		{
			...
			//————————————————————————————————————————————
			const UINT floorIndexSize = sizeof(floorIndices);
			stIBResDesc.Width = floorIndexSize;
			...
			GRS_THROW_IF_FAILED(pID3D12Device4->CreateCommittedResource(
				...
				D3D12_RESOURCE_STATE_GENERIC_READ,
				nullptr,
				IID_PPV_ARGS(&pIIndexBufferFloor)));

			GRS_THROW_IF_FAILED(pIIndexBufferFloor->Map(0, &stReadRange, reinterpret_cast<void**>(&pIndexDataBegin)));
			memcpy(pIndexDataBegin, floorIndices, floorIndexSize);
			pIIndexBufferFloor->Unmap(0, nullptr);

			stIndexBufferViewFloor.BufferLocation = pIIndexBufferFloor->GetGPUVirtualAddress();
			stIndexBufferViewFloor.Format = DXGI_FORMAT_R32_UINT;
			stIndexBufferViewFloor.SizeInBytes = floorIndexSize;
			//————————————————————————————————————————————
		}



		const std::uint32_t vertexBufferSizeCubemap = meshes[1].vertices.size() * sizeof(Vertex);
		const std::uint32_t indexBufferSizeCubemap = meshes[1].indices.size() * sizeof(std::uint32_t);
		//20、创建顶点缓冲
		{
			...
			stResSesc.Flags = D3D12_RESOURCE_FLAG_NONE;
			stResSesc.Format = DXGI_FORMAT_UNKNOWN;
			stResSesc.Width = vertexBufferSizeCubemap;
			stResSesc.Height = 1;
			stResSesc.DepthOrArraySize = 1;
			stResSesc.MipLevels = 1;
			stResSesc.SampleDesc.Count = 1;
			...
			GRS_THROW_IF_FAILED(pID3D12Device4->CreateCommittedResource(
				...
				D3D12_RESOURCE_STATE_GENERIC_READ,
				nullptr,
				IID_PPV_ARGS(&pIVertexBufferCubemap)));

			UINT8* pVertexDataBegin = nullptr;
			D3D12_RANGE stReadRange = { 0, 0 };		// We do not intend to read from this resource on the CPU.
			GRS_THROW_IF_FAILED(pIVertexBufferCubemap->Map(0, &stReadRange, reinterpret_cast<void**>(&pVertexDataBegin)));
			memcpy(pVertexDataBegin, &meshes[1].vertices[0], vertexBufferSizeCubemap);
			pIVertexBufferCubemap->Unmap(0, nullptr);

			stVertexBufferViewCubemap.BufferLocation = pIVertexBufferCubemap->GetGPUVirtualAddress();
			stVertexBufferViewCubemap.StrideInBytes = sizeof(Vertex);//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
			stVertexBufferViewCubemap.SizeInBytes = vertexBufferSizeCubemap;
		}
		...
		//21、创建索引缓冲区		
		{
			...
			stIBResDesc.Dimension = D3D12_RESOURCE_DIMENSION_BUFFER;
			stIBResDesc.Alignment = D3D12_DEFAULT_RESOURCE_PLACEMENT_ALIGNMENT;
			stIBResDesc.Width = indexBufferSizeCubemap;
			stIBResDesc.Height = 1;
			stIBResDesc.DepthOrArraySize = 1;
			stIBResDesc.MipLevels = 1;
			stIBResDesc.Format = DXGI_FORMAT_UNKNOWN;
			...
			GRS_THROW_IF_FAILED(pID3D12Device4->CreateCommittedResource(
				...
				D3D12_RESOURCE_STATE_GENERIC_READ,
				nullptr,
				IID_PPV_ARGS(&pIIndexBufferCubemap)));

			UINT8* pIndexDataBegin = nullptr;
			D3D12_RANGE stReadRange = { 0, 0 };
			GRS_THROW_IF_FAILED(pIIndexBufferCubemap->Map(0, &stReadRange, reinterpret_cast<void**>(&pIndexDataBegin)));
			memcpy(pIndexDataBegin, &meshes[0].indices[0], indexBufferSizeCubemap);
			pIIndexBufferCubemap->Unmap(0, nullptr);

			stIndexBufferViewCubemap.BufferLocation = pIIndexBufferCubemap->GetGPUVirtualAddress();
			stIndexBufferViewCubemap.Format = DXGI_FORMAT_R32_UINT;
			stIndexBufferViewCubemap.SizeInBytes = indexBufferSizeCubemap;
		}
		...
		//23、最终创建SRV描述符   
		{
			...
			auto cubeTexDesc = pICubemap->GetDesc();
			stSRVDesc.Format = cubeTexDesc.Format;
			stSRVDesc.ViewDimension = D3D12_SRV_DIMENSION_TEXTURECUBE;
			stSRVDesc.TextureCube.MipLevels = 1;
			stCPUHandle.Offset(1, nSRVDescriptorSize);
			pID3D12Device4->CreateShaderResourceView(pICubemap.Get(), &stSRVDesc, stCPUHandle);
		}
		...
		while (msg.message != WM_QUIT)
		{
			...
			else
			{
				...
				ImGui::SliderFloat("clearcoatGloss", &(pMVPBuffer->clearcoatGloss), 2.0f, 20.0f);
				
				ImGui::SliderFloat("SphereRadius", &(SphereRadius), 2.0f, 50.0f);
				ImGui::SliderFloat("EyeDistance", &(Radius), 2.0f, 50.0f);
				ImGui::SliderFloat("FloorHight", &(FloorHight), -80.0f, 0.0f);
				...
				//---------------------------------------------------------------------------------------------
				{
					...
					XMMATRIX xmScaleSkyBox = XMMatrixScaling(100.0/ SphereRadius, 100.0 / SphereRadius, 100.0 / SphereRadius);
					...
					//投影矩阵 projection
					xmVP = XMMatrixMultiply(xmVP, (XMMatrixPerspectiveFovLH(XM_PIDIV4, (FLOAT)iWidth / (FLOAT)iHeight, 0.1f, 1000.0f)));

					XMMATRIX xmMVP = XMMatrixMultiply(xmScaleRot, xmVP);
					//计算出旋转矩阵，然后拷贝到上传堆中的常量缓冲区中。
					XMStoreFloat4x4(&pMVPBuffer->m_MVP, xmMVP);

					XMStoreFloat4x4(&pMVPBuffer->m_ObjectToWorldCubemap, xmScaleSkyBox);
					xmMVP = XMMatrixMultiply(xmScaleSkyBox, xmVP);
					XMStoreFloat4x4(&pMVPBuffer->m_MVPSkyBox, xmMVP);

					XMMATRIX xmScaleFloor = XMMatrixScaling(60.0, 1, 60.0);
					xmScaleFloor = XMMatrixMultiply(xmScaleFloor, XMMatrixTranslation(0, FloorHight, 0));
					xmMVP = XMMatrixMultiply(xmScaleFloor, xmVP);
					XMStoreFloat4x4(&pMVPBuffer->m_MVPFloor, xmMVP);
				}
				...
				pICMDList->SetGraphicsRootSignature(pIRootSignatureFloor.Get());
				pICMDList->SetPipelineState(pIPipelineStateFloor.Get());
				ID3D12DescriptorHeap* ppHeaps2[] = { pISRVHeap.Get() };
				pICMDList->SetDescriptorHeaps(1, ppHeaps2);
				stGPUSRVHandle = pISRVHeap->GetGPUDescriptorHandleForHeapStart();
				stGPUSRVHandle.ptr += nSRVDescriptorSize;
				pICMDList->SetGraphicsRootDescriptorTable(0, stGPUSRVHandle);//对应CBV
				pICMDList->IASetPrimitiveTopology(D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
				pICMDList->IASetVertexBuffers(0, 1, &stVertexBufferViewFloor);
				pICMDList->IASetIndexBuffer(&stIndexBufferViewFloor);
				pICMDList->DrawIndexedInstanced(_countof(floorIndices), 1, 0, 0, 0);
				//————————————————————————————————————————————
				//更换PSO是通过reset 命令列表来完成的。

				GRS_THROW_IF_FAILED(pICMDList->Close());
				ID3D12CommandList* ppCommandLists1[] = { pICMDList.Get() };
				pICMDQueue->ExecuteCommandLists(_countof(ppCommandLists1), ppCommandLists1);
				GRS_THROW_IF_FAILED(pICMDList->Reset(pICMDAlloc.Get(), pIPipelineStateCubemap.Get()));

				//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
				//这个视口是绑定到光栅化阶段的，必须和后面代码一起提交。还有下面的裁剪矩形的设置，如果放到Reset前面会直接啥也画不出来。
				//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
				pICMDList->RSSetViewports(1, &stViewPort);
				pICMDList->RSSetScissorRects(1, &stScissorRect);

				////下面需要第二次指定RTV，同时这里reset了RTV的指针，下面就需要一点点修改。
				stRTVHandle = pIRTVHeap->GetCPUDescriptorHandleForHeapStart();
				stRTVHandle.ptr += 3 * nRTVDescriptorSize;
				CD3DX12_CPU_DESCRIPTOR_HANDLE dsvHandle1(pIDSVHeap->GetCPUDescriptorHandleForHeapStart());
				pICMDList->OMSetRenderTargets(2, &stRTVHandle, TRUE, &dsvHandle1);//改为TURE
				//清空操作永远都要在设置完你要清空的渲染目标之后，否则在多个渲染目标的情况下，如果先清空然后再设置RT，清空的就是上一个RT了。

				ID3D12DescriptorHeap* ppHeaps1[] = { pISRVHeap.Get() };
				pICMDList->SetDescriptorHeaps(_countof(ppHeaps1), ppHeaps1);

				pICMDList->SetGraphicsRootSignature(pIRootSignatureCubemap.Get());
				stGPUSRVHandle.ptr += 6 * nSRVDescriptorSize;//SRV
				pICMDList->SetGraphicsRootDescriptorTable(0, stGPUSRVHandle);
				stGPUSRVHandle.ptr -= 6 * nSRVDescriptorSize;//绑定CBV
				pICMDList->SetGraphicsRootDescriptorTable(1, stGPUSRVHandle);
				pICMDList->IASetPrimitiveTopology(D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
				pICMDList->IASetVertexBuffers(0, 1, &stVertexBufferViewCubemap);
				pICMDList->IASetIndexBuffer(&stIndexBufferViewCubemap);

				pICMDList->DrawIndexedInstanced(meshes[1].indices.size(), 1, 0, 0, 0);
				...
				stRTVHandle.ptr -= 3 * nRTVDescriptorSize;//这里修改成3，因为上边重置了一次RTV句柄的指针来第二次指定渲染目标
				stRTVHandle.ptr += (nFrameIndex * nRTVDescriptorSize);
