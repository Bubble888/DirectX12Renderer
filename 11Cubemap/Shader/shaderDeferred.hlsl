...
{
	float4x4 m_MVP;
	float4x4 m_MVPCubemap;
    float4x4 m_MVPFloor;
    float4x4 m_ObjectToWorld;
    float4x4 m_ObjectToWorldCubemap;
    float4 m_ViewPos;
    float3 lightDir;
...
PSInputCubemap VSMainCubemap(VSInput i) 
{
    PSInputCubemap o;
    o.worldPos = mul(float4(i.vertex, 1.0f), m_ObjectToWorldCubemap);
    o.position = mul(float4(i.vertex, 1.0f), m_MVPCubemap);
    return o;
}

PSOutput PSMainCubemap(PSInputCubemap i)
{
    PSOutput o;
    o.halfcolor0 = cubemap.Sample(g_sampler, normalize(i.worldPos.xyz))/2;
...
float4 VSMainFloor(float4 position : POSITION, float4 uv : TEXCOORD) : SV_Position
{
    return mul(position, m_MVPFloor);
}

PSOutput PSMainFloor(float4 position : SV_Position)
{
    PSOutput o;
    o.halfcolor0 = float(0.4);
    o.halfcolor1 = o.halfcolor0;
    return o;
...
PSOutput PSMain(PSInput i) : SV_TARGET
{
    ...
    PSOutput o;
    o.halfcolor0 = float4(BRDF(LightDirection, ViewDirection, NormalDirection, WorldTangent, WorldBinormal, i.uv), 1.0);
    o.halfcolor1 = o.halfcolor0;
    return o;
