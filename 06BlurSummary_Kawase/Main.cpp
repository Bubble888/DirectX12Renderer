...
int APIENTRY _tWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPWSTR    lpCmdLine, int nCmdShow)
{
	...
	try
	{
		...
		//12、创建PSO
		{
			...
			GRS_THROW_IF_FAILED(D3DCompileFromFile(pszShaderFileName, nullptr, nullptr
				, "KawaseBlurCS", "cs_5_0", compileFlags, 0, &pIBlobComputeShaderKawase, nullptr));
		...
		while (msg.message != WM_QUIT)
		{
			...
			else
			{
				...
				//i 为偶数的时候是pIBlurMap0作为CS输入，i为奇数的时候是pIBlurMap1作为CS输入
				for (int i = 0; i < blurTimes; i++) {
					...
					//注意下面传递的，我们需要计算，配合一个组的多少，不然会有些地方没有分配到线程
				...
				if (blurTimes % 2 == 1)//map1
				{
					CD3DX12_RESOURCE_BARRIER N = CD3DX12_RESOURCE_BARRIER::Transition(pIBlurMap1.Get(),
						D3D12_RESOURCE_STATE_GENERIC_READ, D3D12_RESOURCE_STATE_COPY_SOURCE);
					pICMDListArr[nextFrameResource]->ResourceBarrier(1, &N);

					//最后把资源拷贝回到后台缓冲区。
					pICMDListArr[nextFrameResource]->CopyResource(pIARenderTargets[nFrameIndex].Get(), pIBlurMap1.Get());

					CD3DX12_RESOURCE_BARRIER G = CD3DX12_RESOURCE_BARRIER::Transition(pIBlurMap0.Get(),
						D3D12_RESOURCE_STATE_UNORDERED_ACCESS, D3D12_RESOURCE_STATE_COMMON);
					pICMDListArr[nextFrameResource]->ResourceBarrier(1, &G);

					CD3DX12_RESOURCE_BARRIER H = CD3DX12_RESOURCE_BARRIER::Transition(pIBlurMap1.Get(),
